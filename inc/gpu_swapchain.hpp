...
{
  public:
    explicit Swapchain(ImageFormat format) : image_format_(format)
    {
    }
    ...
    std::vector<ImagePtr> const& GetImages() const
    {
        return swapchain_images_;
    }
    ImagePtr const& GetImage(uint32_t index) const
    {
    ...
    ImagePtr GetCurrentImage() const
    {
