    ...
    D3D12DescriptorManager& descriptor_manager, D3D12Descriptor descriptor)
{
    ...
    default:
        assert(
            false && "CopyDescriptorToGPU: source descriptor heap is not shader-visible-copyable");
        return {};
    }
}
} // namespace
    ...
    : device_(device), queue_(queue), command_list_type_(command_list_type)
{
    ...
    ThrowIfFailed(d3d12_device->CreateCommandAllocator(
        command_list_type_, IID_PPV_ARGS(&command_allocator_)));
    ThrowIfFailed(d3d12_device->CreateCommandList(
        0, command_list_type_, command_allocator_.Get(), nullptr, IID_PPV_ARGS(&cmd_list_)));
}
...
void D3D12CommandBuffer::SetVertexBuffer(BufferPtr buffer, size_t vertex_stride)
{
    ...
    {
        throw std::runtime_error("D3D12CommandBuffer::SetVertexBuffer: vertex buffer stride does "
...
void D3D12CommandBuffer::SetIndexBuffer(BufferPtr buffer)
{
    ...
    view.SizeInBytes = static_cast<UINT>(buffer->GetSize());
    view.Format =
        d3d12_buffer->GetStride() == sizeof(uint16_t) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
...
void D3D12CommandBuffer::BindPipeline(GraphicsPipelinePtr const& pipeline)
{
    ...
    {
        throw std::runtime_error("D3D12CommandBuffer::BindPipeline: current vertex buffer stride "
...
void D3D12CommandBuffer::CopyBufferToImage(ImagePtr dst, BufferPtr src)
{
    ...
    device_.GetD3D12Device()->GetCopyableFootprints(
        &image_desc, 0, 1, 0, &footprint, &num_rows, &row_size, &total_size);
...
void D3D12CommandBuffer::BindDescriptorsGraphics()
{
        ...
        current_descriptor_set_->GetBoundDescriptors())
    {
        D3D12Descriptor gpu_descriptor =
            CopyDescriptorToGPU(descriptor_manager, descriptor.cpu_descriptor);
        committed_descriptors_.push_back(gpu_descriptor);

        cmd_list_->SetGraphicsRootDescriptorTable(
            descriptor.root_parameter_index, descriptor_manager.GetGPU(gpu_descriptor));
    }
}
