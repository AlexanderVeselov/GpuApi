...
VulkanBinding const& VulkanDescriptorSet::FindBinding(uint32_t binding, uint32_t space) const
{
    ...
    throw std::runtime_error("VulkanDescriptorSet: pipeline layout binding " +
...
void VulkanDescriptorSet::CreateDescriptorPool()
{
    ...
    VkResult status =
        vkCreateDescriptorPool(device_.GetDevice(), &create_info, nullptr, &descriptor_pool_);
...
void VulkanDescriptorSet::AllocateDescriptorSets()
{
    ...
    VkResult status =
        vkAllocateDescriptorSets(device_.GetDevice(), &allocate_info, descriptor_sets_.data());
