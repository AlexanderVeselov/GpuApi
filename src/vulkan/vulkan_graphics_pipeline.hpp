...
{
public:
    VulkanGraphicsPipelineState(uint32_t width, uint32_t height)
        : width_(width)
        , height_(height)
    {}
    ...
    void SetColorAttachment(uint32_t id, std::shared_ptr<VulkanImage> image)
    {
...
public:
    ...
    void CommitArgs();
    void SetArg(uint32_t set, uint32_t binding, std::shared_ptr<VulkanBuffer> buffer);
    ...
    std::unordered_map<uint32_t, VulkanShader::DescriptorSet> const& GetDescriptorSets() const { return descriptor_sets_; }
...
private:
    ...
    std::unordered_map<uint32_t, VulkanShader::DescriptorSet> descriptor_sets_;
    VkExtent2D extent_;
