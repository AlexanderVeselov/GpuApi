...
{
public:
    VulkanShaderModule(VulkanDevice& device, std::vector<uint32_t> const& spirv)
        : device_(device)
    {
...
VkCompareOp DepthFuncToVkCompareOp(DepthFunc depth_func)
{
    switch (depth_func)
    {
    case DepthFunc::kNone: return VK_COMPARE_OP_ALWAYS;
    case DepthFunc::kNever: return VK_COMPARE_OP_NEVER;
    case DepthFunc::kLess: return VK_COMPARE_OP_LESS;
    case DepthFunc::kEqual: return VK_COMPARE_OP_EQUAL;
    case DepthFunc::kLessEqual: return VK_COMPARE_OP_LESS_OR_EQUAL;
    case DepthFunc::kGreater: return VK_COMPARE_OP_GREATER;
    case DepthFunc::kNotEqual: return VK_COMPARE_OP_NOT_EQUAL;
    case DepthFunc::kGreaterEqual: return VK_COMPARE_OP_GREATER_OR_EQUAL;
    case DepthFunc::kAlways: return VK_COMPARE_OP_ALWAYS;
    default:
...
void GetVertexInputDescs(
    ShaderReflection const& reflection,
...
VulkanGraphicsPipeline::VulkanGraphicsPipeline(
    VulkanDevice& device,
    GraphicsPipelineDesc const& pipeline_desc)
...
void VulkanGraphicsPipeline::Reload()
{
    ...
    VulkanShader vs_shader = shader_manager.CompileShader(pipeline_desc_.vs_filename.c_str(),
        "main", "vs_6_0");
    VulkanShader ps_shader = shader_manager.CompileShader(pipeline_desc_.ps_filename.c_str(),
        "main", "ps_6_0");
    ...
    vertex_input.pVertexBindingDescriptions =
        vertex_attribute_descs.empty() ? nullptr : &vertex_binding_desc;
    vertex_input.vertexAttributeDescriptionCount =
        static_cast<uint32_t>(vertex_attribute_descs.size());
    vertex_input.pVertexAttributeDescriptions =
        vertex_attribute_descs.empty() ? nullptr : vertex_attribute_descs.data();
    ...
    color_blend_state.pAttachments =
        color_blend_attachments.empty() ? nullptr : color_blend_attachments.data();
    ...
    rendering_create_info.pColorAttachmentFormats =
        color_attachment_formats.empty() ? nullptr : color_attachment_formats.data();
    rendering_create_info.depthAttachmentFormat = pipeline_desc_.depth_enabled ?
        ToVkFormat(pipeline_desc_.depth_attachment_format) : VK_FORMAT_UNDEFINED;
...
void VulkanComputePipeline::Reload()
{
    ...
    VulkanShader cs_shader = device_.GetApi().GetShaderManager().CompileShader(
        cs_filename_.c_str(), "main", "cs_6_0");
    ...
    create_info.stage =
        GetShaderStageCreateInfo(VK_SHADER_STAGE_COMPUTE_BIT, cs_module.GetShaderModule());
    create_info.layout = layout_.GetPipelineLayout();

    VkResult status = vkCreateComputePipelines(
        device_.GetDevice(),
        VK_NULL_HANDLE,
        1,
        &create_info,
